"""
Main run script. Generates embeddings, created machine learning models from them
and plots important parameters. Basic grading available with new inputs.
...
generate_embeddings = False # False will read csv embeddings (requires to run once with True). True will generate new embeddings.
embedding_model='text-embedding-ada-002' # See openai documentation for latest model
n_clusters = None  # Determined the number of clusters to use. Set to none to use optimal.
ans_per_cluster = 1 # Number of answers per cluster used to determine naming of cluster.
random_state = None   # Set to none to truly randomize. 42 used in code to reproduce samples to what is in openai docs.
...
for i in range(len(df['Question_ID'].unique())):    
    ...
    ans[i].make_named_clusters(n_clusters=n_clusters, 
                               random_state=random_state, 
                               ans_per_cluster=ans_per_cluster,
                               cluster_description_file=directory+file_answers[:-4]+f'_{q_ID}_named_clusters.csv')
